...
class SeparetedInputFeature(object):
    def __init__(self,
                 ...
                 label: int,
                 mlm_labels: List[int]=None,
                 logits: List[float]=None,
                 meta: Optional[Dict]=None,
                 idx: int=-1):
        # the whole input is divided to 8 part.
        # self.pattern = pattern
        # self.cls_token, self.pre_ids, self.text_a_ids,self.mask_token, self.in_ids, self.text_b_ids, self.post_ids, \
        #     self.eos_and_pad = self.pattern
        self.input_ids = input_ids
        self.prompt_mask, self.text_a_mask, self.text_b_mask, self.text_mask, self.attention_mask = masks
        # self.attention_mask = attention_mask  # for the whole input
        self.token_type_ids = token_type_ids
        self.label = label
        self.mlm_labels = mlm_labels
        self.logits = logits
    ...
    def pretty_print(self, tokenizer):
        return f'input_ids         = {self.pattern}' + \
               f'attention_mask    = {self.attention_mask}\n' + \
               f'token_type_ids    = {self.token_type_ids}\n' + \
               f'mlm_labels        = {self.mlm_labels}\n' + \
               f'logits            = {self.logits}\n' + \
