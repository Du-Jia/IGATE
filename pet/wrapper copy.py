...
class TransformerModelWrapper:
                    ...
                    if global_step % self.config.eval_every_step == 0:
                        ...
                        if self.config.task_name in ["cb", "record", "multirc"]:
                            ...
                            if dev32_scores["acc"] >= best_dev32_acc:
                                ...
                                logger.info(dev32_scores)
                                logger.info(early_stop_epoch)
                        elif self.config.task_name in ['mrpc', 'qqp']:
                            if dev32_scores["acc"] >= best_dev32_acc:
                                if dev32_scores["acc"] > best_dev32_acc:
                                    early_stop_epoch = 0
                                else:
    ...
    def generate_instance_level_prompt(self, batch: Dict[str, torch.Tensor]) -> Dict[str, torch.Tensor]:
        ...
        for bidx in range(bz):
            ...
            elif info_type == 'attention':
                # TODO: use self-attention
                # calculate attention scores
                # 
                attention_scores = torch.softmax(torch.matmul(raw_embeds, raw_embeds.transpose(2, 1))/torch.sqrt(raw_embeds.shape[-1]), dim=1)
                context_info = attention_scores
                # context_replace_embeds = replace_embeds + gamma * max_sent
                context_replace_embeds = replace_embeds + model.gamma * max_sent
            elif info_type == 'bert-like':
                # TODO: use bert-like attention
                # (prompt embeddings will be used when calculate attention scores)
                bert_like_info = torch.max(raw_embeds[bidx], dim=0).values
                context_replace_embeds = replace_embeds + model.gamma * bert_like_info
            elif info_type == 'none':
                context_replace_embeds = replace_embeds
            else:
