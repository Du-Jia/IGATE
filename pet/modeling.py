...
def train_pet(train_data: List[InputExample],
              ...
              output_dir: str,
              tag: str,  # add by du jia, 用于指定输出结果的文件名
              repetitions: int = 1,
              do_train: bool = True,
              do_eval: bool = True,
              seed: int = 42
    ...
    :param tag: the output result file name information
    ...
    parameters_info = {}
    parameters_info['SEED'] = seed
    parameters_info['LR'] = train_config.learning_rate
    parameters_info['BS'] = train_config.per_gpu_train_batch_size * train_config.n_gpu
    parameters_info['PL'] = pattern_ids[0]
    parameters_info['GAMMA_TRAINABLE'] = model_config.trainable_gamma
    parameters_info['GAMMA'] = model_config.gamma
    parameters_info['INFO'] = model_config.info_type
    parameters_info['USE_CONV'] = model_config.use_conv
        ...
        for iteration in range(repetitions):
            ...
            if do_eval:
                ...
                with torch.no_grad():
                    ...
                    save_predictions(os.path.join(pattern_iter_output_dir, f'eval_predictions_{t}.jsonl'), wrapper, eval_result)
                    save_logits(os.path.join(pattern_iter_output_dir, f'eval_logits_{t}.txt'), eval_result['logits'])
    ...
    if do_eval:
        ...
        # add by du jia.
        result_file = f'{tag}.txt'
        _write_results(os.path.join(output_dir, result_file), results, dev32_results, parameters_info)
        # _write_results(os.path.join(output_dir, 'result_test.txt'), results, dev32_results, parameters_info)
...
def _write_results(path: str, all_results: Dict, dev32_results: Dict, parameters_info: Dict):
    with open(path, 'a+') as fh:  # Be modified by du jia, 每次的实验结果都存到一起
        time_info = time.strftime("%Y-%m-%d %H:%M:%S", time.localtime(time.time()))
        fh.write('==== [{}]-Parameters: SEED={}, LR={}, BS={}, PL={}, GAMMA_TRAINABLE={}, GAMMA_INIT={}, USE_CONV={} ====, INFO_TYPE={}\n'.format(time_info,
                                                                                  parameters_info['SEED'],
                                                                                  parameters_info['LR'],
                                                                                  parameters_info['BS'],
                                                                                  parameters_info['PL'],
                                                                                  parameters_info['GAMMA_TRAINABLE'],
                                                                                  parameters_info['GAMMA'],
                                                                                  parameters_info['INFO'],
                                                                                  parameters_info['USE_CONV']))
        results = all_results
        logger.info("eval_results:")
