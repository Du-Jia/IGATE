...
from data_utils.task_processors import (
    BoolQProcessor,
    CbProcessor,
    CopaProcessor,
    MultiRcProcessor,
...
from data_utils.custom_task_processor import PROCESSORS
from data_utils.task_processors import load_examples, DEV32_SET, TRAIN_SET, DEV_SET, TEST_SET, METRICS, DEFAULT_METRICS
...
def load_pet_configs(args) -> Tuple[WrapperConfig, TrainConfig, EvalConfig]:
    ...
    model_cfg = WrapperConfig(model_type=args.model_type,
                              ...
                              info_type=args.info_type,  # add by du jia
                              trainable_gamma=args.trainable_gamma,  # add by du jia
                              gamma=args.gamma,  # add by du jia
                              tag=args.tag,
...
def main():
    ...
    # add by du jia
    parser.add_argument("--info_type", default='max', type=str)
    # 为了方便设置，直接设置为字符串类型
    parser.add_argument("--trainable_gamma", default='True', type=str, help="If True, set gamma trainable")
    parser.add_argument("--gamma", default=0.001, type=float, help="initial value of gamma")
    # 每次实验的tag，最好由每次实验的自变量组成
    parser.add_argument("--tag", default='info_type', type=str, help='result file name')
    parser.add_argument("--eval_protocol", default='dev32', type=str, help='result file name')
    parser.add_argument('--use_conv', action='store_true', help="Extract information with convolution")
    ...
    logger.info("Model Configs: {}".format(pet_model_cfg))
    logger.info("Train Configs: {}".format(pet_train_cfg))
    logger.info("Eval Configs: {}".format(pet_eval_cfg))

    print("[DEBUG] ==== Info Start ==== [DEBUG]\n")
    print(train_data[0])
    print(dev32_data[0])
    print(eval_data[0])
    print("[DEBUG] ==== Info End ==== [DEBUG]\n")
    ...
    train_pet(eval_data=eval_data,
              ...
              output_dir=args.output_dir,
              tag=args.tag,  # add by du jia. 用于指定输出结果的文件名
